		                            ...
		                            <div class="box-body">
	                                    <form role="form">
		                                    ...
		                                    <div class="form-group">
								            	...
								            	<button id="addAll" type="button" class="btn bg-olive">添加所有</button>
								            </div>
		                                    <div class="form-group">
		                                    	<label>已关联的店铺</label>
												<select id="restaurantList" multiple class="form-control">
												</select>
		                                    </div>
		                                    <div class="form-group">
								            	<button id="remove" type="button" class="btn bg-olive">移除</button>
								            	<button id="removeAll" type="button" class="btn bg-olive">移除所有</button>
								            </div>
								        </form>
								    </div>
	                            </div><!-- /.box -->
        ...
        <script type="text/javascript">
            ...
            $(function() {
				...
				$("button#save").click(function(){
					...
					if(id == 0) {
						ajax("web/distribution/createdistribution",{
							name:name,
							address:address,
							tel:tel
						},function(data){
						...
						ajax("web/distribution/updatedistribution",{
							distributionId:id,
							name:name,
							address:address,
							tel:tel
				...
				$("button#add").click(function(){
					var id = ${id};
					var restaurantId = $("select#restaurant").val();
					if(restaurantId == 0) {
						alert("请选择要添加的店铺。");
						return;
					}
					ajax("web/distribution/add", {distributionId:id, restaurantId:restaurantId}, function(data){
						refresh(id);
						alert(data.msg);
					});
				});
				$("button#addAll").click(function(){
					var id = ${id};
					ajax("web/distribution/addall", {distributionId:id}, function(data){
						refresh(id);
						alert(data.msg);
					});
				});
				$("button#remove").click(function(){
					var id = ${id};
					var restaurantId = $("select#restaurantList").val();
					if(restaurantId == null) {
						alert("请选择要移除的店铺。");
						return;
					}
					ajax("web/distribution/remove", {distributionId:id, restaurantId:restaurantId[0]}, function(data){
						refresh(id);
						alert(data.msg);
					});
				});
				$("button#removeAll").click(function(){
					var id = ${id};
					ajax("web/distribution/removeall", {distributionId:id}, function(data){
						refresh(id);
						alert(data.msg);
					});
				});
            ...
            function refresh(id) {
            	ajax("web/distribution/distributiondetail",{distributionId:id},function(data){
					if(data.ret != 0) {
						alert(data.msg);
						return;
					}
						...
						var list = data.data.restaurant;
						$("select#restaurantList").empty();
						for(var i = 0;i < list.length;i++) {
							$("select#restaurantList").append('<option value="'+list[i].id+'">'+list[i].name+'</option>');
						}
					}
				});
            }
